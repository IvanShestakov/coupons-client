...
export class CustomerCouponService {
  ...
  private couponsURL = this.baseURL.concat('/coupons');  

  constructor(private http: Http) { }

  getAllPublishedCoupons(): Promise<Coupon[]> {
    return this.http
              .get(this.couponsURL)
              .toPromise()
  ...
  getCustomerCoupons(): Promise<Coupon[]> {
    let url = `${this.couponsURL}/customer/${sessionStorage.getItem('userId')}`;
...
/*    let coupons: Coupon[] = [
  ...
  purchaseCoupon(id: number): Promise<Coupon> {
      let url = `${this.couponsURL}/customer/${sessionStorage.getItem('userId')}/${id}`;
      return this.http
